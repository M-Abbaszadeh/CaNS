...
contains
       ...
       do j=1,n(2)
          do i=1,n(1)
             ...
             vox(i,j,k) = 0.25_rp*( &
                                   ...
                                   (uz(i,j+1,k-1)-uz(i,j  ,k-1))*dyi - (uy(i,j  ,k  )-uy(i,j  ,k-1))*dzci(k) + &
                                   (uz(i,j  ,k  )-uz(i,j-1,k  ))*dyi - (uy(i,j-1,k+1)-uy(i,j-1,k  ))*dzci(k) + &
                                   (uz(i,j  ,k-1)-uz(i,j-1,k-1))*dyi - (uy(i,j-1,k  )-uy(i,j-1,k-1))*dzci(k) &
                                  )
             !
             ...
             voy(i,j,k) = 0.25_rp*( &
                                   ...
                                   (ux(i  ,j,k  )-ux(i  ,j,k-1))*dzci(k) - (uz(i+1,j,k-1)-uz(i  ,j,k-1))*dxi + &
                                   (ux(i-1,j,k+1)-ux(i-1,j,k  ))*dzci(k) - (uz(i  ,j,k  )-uz(i-1,j,k  ))*dxi + &
                                   (ux(i-1,j,k  )-ux(i-1,j,k-1))*dzci(k) - (uz(i  ,j,k-1)-uz(i-1,j,k-1))*dxi &
                                  )
             !
  ...
  subroutine strain_rate(n,dli,dzci,ux,uy,uz,str)
    implicit none
    integer , intent(in ), dimension(3)        :: n
    real(rp), intent(in ), dimension(3)        :: dli
    real(rp), intent(in ), dimension(0:)       :: dzci
...
#else
    !$OMP PARALLEL DEFAULT(none) &
    !$OMP SHARED(n,dxi,dyi,dzci,ux,uy,uz,str) &
    ...
    do k=1,n(3)
       do j=1,n(2)
          do i=1,n(1)
             ...
             s33 = ((uz(i,j,k)-uz(i,j,k-1))*dzci(k))**2
             s12 = .25_rp*( &
             ...
             s13 = .25_rp*( &
                            ...
                            ((ux(i  ,j,k  )-ux(i  ,j,k-1))*dzci(k) + (uz(i+1,j,k-1)-uz(i  ,j,k-1))*dxi)**2 + &
                            ((ux(i-1,j,k+1)-ux(i-1,j,k  ))*dzci(k) + (uz(i  ,j,k  )-uz(i-1,j,k  ))*dxi)**2 + &
                            ((ux(i-1,j,k  )-ux(i-1,j,k-1))*dzci(k) + (uz(i  ,j,k-1)-uz(i-1,j,k-1))*dxi)**2 &
                          )*.25_rp
             s23 = .25_rp*( &
                           ((uy(i,j  ,k+1)-uy(i,j  ,k  ))*dzci(k) + (uz(i,j+1,k  )-uz(i,j  ,k  ))*dyi)**2 + &
                           ((uy(i,j  ,k  )-uy(i,j  ,k-1))*dzci(k) + (uz(i,j+1,k-1)-uz(i,j  ,k-1))*dyi)**2 + &
                           ((uy(i,j-1,k+1)-uy(i,j-1,k  ))*dzci(k) + (uz(i,j  ,k  )-uz(i,j-1,k  ))*dyi)**2 + &
                           ((uy(i,j-1,k  )-uy(i,j-1,k-1))*dzci(k) + (uz(i,j  ,k-1)-uz(i,j-1,k-1))*dyi)**2 &
                          )*.25_rp
